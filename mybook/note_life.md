...
我要学习下树状数组 和 线段树.还应该学习一下网络流,这些常用的算法都应该好好
...
many admin parts of the projects. I should find works by myself, my manager is so busy.
...
读完了 《明朝那些事》 很不错的书。有不少想法，好多事情，不是你想的那样，道理还有很多，并且
...
我应该继续学习前端的东西，jquery 之类的，以后要搞网站的话，这些是离不开的。
...
看了一些c语言的题。感觉不是很难，但是对我的变成能力是很有帮助的。我应该找时间多看看。
...
他回来以后，拿来了自己的电脑，是一个ThinkX220i。刚开始我还有点纳闷，怎么不是mac，还有一点鄙视的情绪。
...
I want to finish my Btree. I have to finished it.
...
哈哈，其实最主要的就是 kde 中 的 gtk 的选择的问题。gtk 应该选择 gnome 的那个默认的才行。
上面已有描述。

对每一个篇文章，应该都有一个简要的 summary。

你可以保留原来的oxgen，只给 eclipse gnome 的这个gtk。按照下面这样做。
env GTK2_RC_FILES="/usr/share/themes/Adwaita/gtk-2.0/gtkrc" /home/user/Zend/ZendStudio/ZendStudio
env GTK2_RC_FILES="" /home/user/Zend/ZendStudio/ZendStudio 
two new tools.
apt-file
http://www.cnblogs.com/ahuo/archive/2012/07/25/2608172.html
apt-show-versions

2015-01-25 04:29
昨晚喝了不少酒，并且都喝醉了。下次还是收敛一点，太过了不好。
感觉自己的状态不是太好，应该调整下。认识如何走向灭亡的，度<明朝那些事>的时候，看到一些人因为
自满一类的感觉走向失败，很是不理解，觉着不应该，现在看来，还是很可怕的，在你明白的时候，已经
来不及了，所以一定要保持警惕的态度。不要膨胀过度。

我要继续学着写代码，我的blog应该继续更新下去，我要想办法写更好的文章，而不是一直像现在这个样子。
Python 的异常，我也不是怎么熟悉。有时间应该多了解下这些系统函数是怎么实现的。这是一个不错的学习
方式。可以学习的源代码又很多，应该多学习。不要自满。


不知道我一天都在想写什么,为什么这么乱。
PHP 的 static 应该是不一样的。
PHP 的字符串可以这么用，我也是长见识了。
echo "hello{$world";}

下面是一些常用的方法，是我从框架中看到的。
parse_url();
strtok();
array_flip();
preg_match();
...
serialize();
print_r();
getmypid();得到PHP进程的PID。

不知道PHP的函数都实现在什么地方，为什么我看到的都是函数申明。看不到函数的具体实现。
...
这个popen好像是在 python 的文档中发现的，p 一定是 pipe 的意思。

try {
    $code = $connection->incr(self::KEY_TICKETER);
    return $code . intval($serverId);
} catch (\Exception $ex) {
    unset($config[$serverId]);
