...
2. 更清楚业务。
3. 相关系统的服务化。
4. 话说那些工作需要我做，会有什么不一样。负责bug群里的问题. redmine飞龙一律指给我。
5. 将需求指给我。听需求会，负责安排任务，排期 ==> 交给飞龙。
6. 开发过程中负责和各方面的资源进行协调。协调客户端，协调测试，协调其他部门,把我项目的进度。
...
我应该总结一下自己的工作. 好像下面就这些工作内容.

熟悉Linux开发环境.熟悉shell和shell下的的工具.vim,awk,tail,cat,grep,find .
可以写一些shell脚本来辅助开发.比如一个wrap_git 的脚本来简化代码提交的脚本,可以提高提交代码的提交效率.
一个redis查询脚本,可以快速定位自己想要的key值,来对开发中遇到的case进行快速分析.对常用的命令进行alias,
这样可以省去很多敲复杂命令的时间.开发讲代码部署到测试server的脚本.
负责组内代码的合并,review,上线.开发合并代码的脚本,减少合并代码的出错可能性.
对经常出现的问题,会开发一定的脚本来进行定位问题.辅助解决问题,提高解决问题的效率.
熟悉nmp,可以搭建基本的开发环境,基于linux,nginx,mysql.配置虚拟主机,辅助开发.
熟悉mysql,对mysql索引有比较深刻的理解.
熟悉PHP框架,Yii,基于路由的PHP框架,抽象提取公共方法,参与项目框架的改进与优化.
熟悉markdown,有写项目接口文档的习惯.

负责开发我们的IM(聊天)推送系统.
Python , redis , mysql , 消息队列 , 信鸽sdk .
参与项目架构的演进,项目从单库向多库的拆分,单库变7个库.(前期方案设计,到后期实现)
参与数据库分表.

开发APP数据接口(商品列表数据,商品详情数据,用户数据,资源位{设计数据模型并实现}).
设计并实现活动促销系统,在促销期间,商品按照促销价格售卖,促销结束,商品恢复原价{平台促销的主要方式}.
开发相关的数据运营后台.供运营人员使用.(Yii 框架,熟悉js,juqery,ajax).
