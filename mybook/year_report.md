...
优化接口的访问性能,给接口增加缓存层(redis),提高核心接口的qps,降低接口的相应时间. 
开发相关的数据运营后台.供运营人员使用.(Yii 框架,js,juqery,ajax).

对接产品需求,进行项目估期,分配任务,跟进项目进度,协调各方面资源,保证按期发版.
负责线上bug的安排处理.(自己处理,安排给组员处理).
支持运营产品的临时需求(未排期需求).
